<template>
  <div class="compatibility-page">
    <v-container class="compatibility-container">
      <!-- Breadcrumb -->
      <nav class="breadcrumb-row mb-6 mt-2 text-body-2 bidi-text" aria-label="Navigatiepad">
        <router-link to="/nl/" class="text-decoration-none"><span class="text-medium-emphasis">Home</span></router-link>
        <span class="mx-2">></span>
        <router-link to="/nl/guides/install-esim-iphone" class="text-decoration-none"><span class="text-medium-emphasis">eSIM-handleidingen</span></router-link>
        <span class="mx-2">></span>
        <strong>Compatibele toestellen</strong>
      </nav>

      <div class="mb-8">
        <h1 class="section-title mb-4 bidi-text">Controleer de compatibiliteit</h1>

        <p class="page-text mb-4 bidi-text"> Om een Safar Sim eSIM te gebruiken, moet je toestel aan deze voorwaarden voldoen: </p>
          ...
          <v-col class="" cols="12">
            <h3 class="condition-title bidi-text">
              <v-icon
                ...
                icon="mdi-check"
                size="large"
              /> Het toestel ondersteunt eSIM. </h3>
          </v-col>
          ...
          <v-col class="" cols="12">
            <h3 class="condition-title bidi-text">
              <v-icon
                ...
                icon="mdi-check"
                size="large"
              /> Het toestel heeft geen simlock van een provider of netwerk. </h3>
          </v-col>
          ...
          <v-col class="" cols="12">
            <h3 class="condition-title bidi-text">
              <v-icon
                ...
                icon="mdi-check"
                size="large"
              /> Het toestel is niet gejailbreakt op iOS of geroot op Android. </h3>
          </v-col>
        </v-row>

        <p class="page-text mb-4 bidi-text"> Bekijk onze lijst om te controleren of je toestel eSIM ondersteunt. Sommige regionale modellen ondersteunen mogelijk geen eSIM. </p>

        <v-card rounded="xl" elevation="1" class="pa-5 compatibility-card mb-8">
          <h4 class="subsection-title mb-4 bidi-text">Zoek compatibele telefoons</h4>
          <v-text-field
            v-model.trim="search"
            label="Zoek op merk of model"
            variant="outlined"
            density="comfortable"
            rounded="lg"
          ...
          <div class="phone-results" role="list">
            ...
            <p v-if="filteredPhoneOptions.length === 0" class="phone-empty">Geen compatibele telefoon gevonden</p>
          </div>
          <v-alert v-if="selectedPhone" type="success" variant="tonal" class="mt-4 bidi-text">
            <strong>Compatibele telefoon geselecteerd:</strong>
          </v-alert>
          <router-link v-if="selectedPhone" :to="{ path: '/', hash: '#destination-selection' }" class="destination-cta-link">
            <v-btn color="pink" size="large" class="text-none destination-cta" prepend-icon="mdi-earth">Bekijk bestemmingen</v-btn>
          </router-link>
        </v-card>
