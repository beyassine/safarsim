      ...
      <v-container class="hero-container">
        <div class="hero-copy">
          ...
          <div class="hero-actions">
            ...
            <router-link to="/compatibility" class="compatibility-link">
              <v-icon size="21">mdi-cellphone-check</v-icon>
              {{ c.checkPhone }}
            </router-link>
          </div>
          <div class="hero-trust">
            <span><v-icon>mdi-check-circle</v-icon> {{ c.instant }}</span>
            <span><v-icon>mdi-check-circle</v-icon> {{ c.noFees }}</span>
            <span><v-icon>mdi-check-circle</v-icon> {{ c.support }}</span>
          </div>
        </div>
        ...
        <div class="hero-visual" :aria-label="c.connected">
          ...
          <div class="phone-shell">
            ...
            <div class="phone-screen">
              <div class="signal-row"><span>9:41</span><span><v-icon size="16">mdi-signal</v-icon> 5G <v-icon size="18">mdi-battery-high</v-icon></span></div>
              <img :src="logo" class="screen-logo" alt="SafarSIM">
              <div class="connected-icon"><v-icon size="46">mdi-wifi</v-icon></div>
              <strong>{{ c.connected }}</strong>
              <small>{{ c.internetReady }}</small>
              <div class="usage-card"><span>{{ c.remaining }}</span><b>4.8 GB</b><div><i></i></div></div>
            </div>
          </div>
          <div class="floating-card flag-card">🌍 <span>{{ c.moreThan }}<br><b>190 {{ c.destination }}</b></span></div>
          <div class="floating-card speed-card"><v-icon>mdi-flash</v-icon><span>{{ c.fastNetwork }}<br><b>4G / 5G</b></span></div>
        </div>
      </v-container>
    </section>
    ...
    <section id="plans" class="plans-section">
      <v-container>
        ...
        <div class="purchase-card">
          ...
          <div id="package-selection" class="package-panel">
            <div class="step-label"><i>2</i><span>{{ c.selectPackage }}</span></div>
            <div v-if="selectedDestination" class="selected-country">
            ...
            <div class="package-grid">
              <button v-for="plan in availablePlans" :key="plan.key" type="button" class="plan-option"
                ...
                <strong>{{ formatPrice(plan.price) }} <small>{{ c.mad }}</small></strong>
...
const initialDestination = regions.find(item => item.slug === 'europe') || destinations[0]
const selectedDestination = ref(initialDestination)
...
const pc = computed(() => popularCopy[locale.value] || popularCopy.fr)
...
}[locale.value] || 'Du choix de votre forfait à la connexion, tout se fait rapidement et simplement.'))
const catalogCopy = {
  ar: { regions: 'الباقات الإقليمية', countries: 'الدول', countryCount: 'دولة' },
  fr: { regions: 'Forfaits régionaux', countries: 'Pays', countryCount: 'pays' },
  en: { regions: 'Regional plans', countries: 'Countries', countryCount: 'countries' },
}
const cc = computed(() => catalogCopy[locale.value] || catalogCopy.fr)

const popularPacks = computed(() => ['france', 'espagne', 'turquie', 'europe']
  .map(slug => catalog.find(item => item.slug === slug))
  .filter(Boolean)
  .map(destination => {
    const preferredKey = destination.plans?.['5GB_30days']?.price != null ? '5GB_30days' : Object.keys(destination.plans || {})[0]
...
const destinationGroups = computed(() => [
  ...
  },
  {
    ...
    items: filteredDestinations.value.filter(item => item.type !== 'region'),
  },
...
function flagUrl(destination) {
  try {
    ...
    return require(`@/assets/images/flags/${destination.iso.toLowerCase()}.svg`)
  } catch (error) {
...
function formatPrice(price) {
  ...
  return new Intl.NumberFormat(numberLocale, { maximumFractionDigits: 2 }).format(price)
}
...
onMounted(() => {
  ...
  if (availablePlans.value.length) selectedPlan.value = availablePlans.value[Math.min(2, availablePlans.value.length - 1)]
})
...
<style scoped>
.one-page { --ink:#2f1b2a; --green:#d91c58; --mint:#fde7f0; --coral:#e72a64; --cream:#fffbf8; background:#fffbf8; color:var(--ink); overflow:hidden; font-family:inherit; }
...
.compatibility-link { display:flex;align-items:center;gap:7px;color:var(--ink);font-weight:700;text-decoration:none;border-bottom:1px solid #a7b5b2;padding-bottom:4px; }
.hero-trust { display:flex;gap:22px;flex-wrap:wrap;margin-top:30px;color:#5b6e69;font-size:14px; }
.hero-trust span { display:flex;align-items:center;gap:5px; }.hero-trust .v-icon { color:#d91c58;font-size:18px; }
.one-page[dir="rtl"] .hero-trust span{direction:ltr;flex-direction:row-reverse}
...
.search-wrap { height:48px;background:#fff;border:1px solid #d8e3df;border-radius:12px;padding:0 13px;display:flex;align-items:center;gap:8px;margin-bottom:12px;color:#83928e; }.search-wrap input{border:0;outline:0;background:transparent;width:100%;font:inherit;color:var(--ink)}
.destination-list{display:flex;flex-direction:column;gap:5px;max-height:390px;overflow:auto;padding-inline-end:4px}.destination-option{border:1px solid transparent;background:transparent;border-radius:11px;padding:10px;display:flex;gap:10px;align-items:center;text-align:start;color:var(--ink);cursor:pointer;font:inherit}.destination-option>span{flex:1;text-align:start}.destination-option:hover{background:#fff}.destination-option.active{background:#fff;border-color:#e990ae;box-shadow:0 5px 14px rgba(92,28,62,.07);font-weight:800}.destination-option img,.selected-country img{width:28px;height:20px;flex:0 0 28px;object-fit:cover;border-radius:3px;box-shadow:0 1px 3px #aaa}.destination-option .v-icon{color:var(--green);margin-inline-start:auto}.one-page[dir="rtl"] .destination-option,.one-page[dir="rtl"] .selected-country>div{direction:rtl}.one-page[dir="ltr"] .destination-option,.one-page[dir="ltr"] .selected-country>div{direction:ltr}.no-results{text-align:center;color:#806d78;margin:30px 0}
.destination-group+.destination-group{margin-top:14px;padding-top:13px;border-top:1px solid #eadfd9}.destination-group-title{position:sticky;top:0;z-index:2;display:flex;align-items:center;gap:7px;margin-bottom:6px;padding:7px 8px;border-radius:9px;background:#fffbf8;color:#765f6d;font-size:12px}.destination-group-title strong{font-size:13px;color:var(--ink)}.destination-group-title>span{margin-inline-start:auto;min-width:25px;text-align:center;background:#fff;border:1px solid #e8ddd7;border-radius:99px;padding:2px 6px;font-weight:800}.destination-group--regions .destination-group-title{background:#fff0f5;color:#d91c58}.destination-option.region-option{margin-bottom:4px;background:#fff;border-color:#f1d5df}.destination-option.region-option:hover,.destination-option.region-option.active{border-color:#d91c58}.destination-option>small{color:#9a7886;font-size:10px;white-space:nowrap}.one-page[dir="rtl"] .destination-group-title{direction:rtl}.one-page[dir="ltr"] .destination-group-title{direction:ltr}
.destination-group{display:flex;flex-direction:column;width:100%}.destination-group .destination-option{width:100%;max-width:460px}.one-page[dir="rtl"] .destination-group .destination-option{margin-left:auto;margin-right:0}.one-page[dir="ltr"] .destination-group .destination-option{margin-left:0;margin-right:auto}
...
.selected-country { display:flex;justify-content:space-between;align-items:center;padding:13px 15px;border-radius:12px;background:#fffbf8;margin-bottom:16px;gap:12px}.selected-country>div{display:flex;gap:10px;align-items:center;font-weight:800}.selected-country small{color:#806572}
...
.checkout-section{padding:70px 0 24px;background:#fffbf8;scroll-margin-top:65px}.checkout-heading{margin-bottom:20px}.embedded-checkout{max-width:1140px;margin:auto}.embedded-checkout :deep(.cart-page){min-height:0!important;padding-top:12px!important;padding-bottom:8px!important}.embedded-checkout :deep(.summary-card),.embedded-checkout :deep(.compatibility-card),.embedded-checkout :deep(.contact-card),.embedded-checkout :deep(.payment-card){border:1px solid #eee5df!important;box-shadow:0 14px 35px rgba(92,28,62,.06)!important}.embedded-checkout :deep(.v-btn.bg-green-darken-1){background:#d91c58!important;color:#fff!important}.embedded-checkout :deep(.subsection-title){font-family:inherit!important;color:var(--ink)}
...
.one-page[dir="rtl"] .embedded-checkout :deep(.v-radio){width:100%;direction:rtl}.one-page[dir="rtl"] .embedded-checkout :deep(.v-radio .v-label){width:100%;max-width:none;flex:1;direction:rtl;text-align:right;justify-content:flex-start}.one-page[dir="rtl"] .embedded-checkout :deep(.v-radio .v-label>div){width:100%;direction:rtl;text-align:right}
.one-page[dir="rtl"] .embedded-checkout :deep(.payment-card .d-flex.justify-space-between){flex-direction:row-reverse}.one-page[dir="ltr"] .embedded-checkout :deep(.payment-card .d-flex.justify-space-between){flex-direction:row}
.one-page[dir="rtl"] .embedded-checkout :deep(.summary-card>.d-flex.justify-space-between.mb-4){flex-direction:row-reverse}.one-page[dir="ltr"] .embedded-checkout :deep(.summary-card>.d-flex.justify-space-between.mb-4){flex-direction:row}
...
@media(max-width:960px){.hero-container{grid-template-columns:1fr;text-align:center;padding-top:75px}.hero-lead{margin-inline:auto}.hero-actions,.hero-trust{justify-content:center}.hero-visual{height:470px}.purchase-card{grid-template-columns:1fr;gap:22px}.destination-list{display:grid;grid-template-columns:1fr 1fr;max-height:260px}.benefits-layout{grid-template-columns:1fr;gap:45px}.steps-grid{grid-template-columns:1fr}.step-arrow{transform:rotate(-90deg)}.proof-grid{grid-template-columns:1fr 1fr;gap:20px}.proof-grid div{border:0}.final-cta .v-container{flex-direction:column;text-align:center}}
@media(max-width:960px){.popular-packs-grid{grid-template-columns:1fr 1fr}}
@media(max-width:600px){.page-language-bar{padding:10px 0 3px}.language-bar-inner{flex-direction:column;justify-content:center;gap:7px;padding-inline:8px}.language-prompt{display:flex;font-size:12px;gap:5px}.language-prompt .v-icon{font-size:17px!important}.language-options{width:auto;justify-content:center;gap:5px;min-width:0}.language-options button{min-width:0;padding:6px 8px;font-size:11px;gap:4px}.hero-section{min-height:auto}.hero-container{padding-top:24px;padding-bottom:50px}.hero-copy h1{font-size:40px;letter-spacing:-1px}.hero-lead{font-size:16px}.hero-actions{flex-direction:column}.hero-trust{gap:10px}.hero-visual{height:410px;transform:scale(.88);margin:-20px}.phone-shell{height:445px}.orbit-one{width:380px;height:380px}.orbit-two{width:300px;height:300px}.flag-card{right:-25px}.speed-card{left:-25px}.proof-grid b{font-size:18px}.plans-section,.how-section,.benefits-section,.faq-section{padding:70px 0}.section-heading{margin-bottom:30px}.purchase-card{border-radius:18px}.destination-panel,.package-panel{padding:20px}.destination-list{grid-template-columns:1fr}.package-grid{grid-template-columns:1fr}.selected-country{align-items:flex-start;flex-direction:column}.one-page[dir="rtl"] .selected-country>div,.one-page[dir="rtl"] .selected-country small{width:100%;text-align:right}.one-page[dir="rtl"] .selected-country>div{justify-content:flex-start}.one-page[dir="ltr"] .selected-country>div,.one-page[dir="ltr"] .selected-country small{width:100%;text-align:left}.checkout-row{align-items:stretch;flex-direction:column}.buy-button{width:100%}.benefit-copy h2{font-size:31px}.benefit-grid{grid-template-columns:1fr}.final-cta h2{font-size:28px}.final-cta .v-btn{width:100%}}
...
.compact-steps-section{background:#fff;padding:8px 0 24px}.compact-steps-heading{margin-bottom:27px}.compact-steps-heading h2{font-size:clamp(27px,3.5vw,38px)}.compact-steps{max-width:820px;margin:auto;display:grid;grid-template-columns:1fr;gap:10px}.compact-step{min-height:86px;display:flex;align-items:center;gap:16px;padding:13px 20px;background:#fff;border:1px solid #e1e3e6;border-radius:17px;box-shadow:0 7px 16px rgba(48,28,39,.07);color:var(--ink);direction:ltr}.compact-step i{width:37px;flex:0 0 37px;color:#d91c58;font-size:21px;font-weight:900;font-style:normal;text-align:center}.compact-step>.v-icon{width:42px;flex:0 0 42px;color:#2f1b2a;font-size:36px}.compact-step>div{flex:1}.compact-step strong{display:block;font-size:15px;line-height:1.4}.compact-step p{margin:4px 0 0;color:#786873;font-size:12px;line-height:1.55;font-weight:400}.one-page[dir="rtl"] .compact-step>div{direction:rtl;text-align:right}.one-page[dir="ltr"] .compact-step>div{direction:ltr;text-align:left}
