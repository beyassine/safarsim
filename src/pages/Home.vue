<template>
  <div class="one-page" :class="{ 'locale-fr': locale === 'fr', 'locale-en': locale === 'en' }" :dir="pageDirection">
          ...
          <div v-if="selectedDestination" id="package-selection" class="package-panel">
            ...
            <div class="package-grid">
              <button v-for="plan in availablePlans" :key="plan.key" type="button" class="plan-option"
                ...
                <div class="plan-data"><b>{{ plan.data }}</b></div>
...
.hero-glow-one { width:380px;height:380px;background:rgba(248,202,217,.32);right:-120px;left:auto;top:-100px; }
.hero-glow-two { width:260px;height:260px;background:rgba(179,225,207,.25);left:-90px;right:auto;bottom:-100px; }
.hero-glow-three { width:390px;height:390px;background:rgba(252,224,233,.2);right:-135px;bottom:-125px; }
...
.package-grid{display:grid;grid-template-columns:1fr 1fr;gap:11px}.plan-option{min-height:118px;position:relative;background:#fff;border:1.5px solid #eadde3;border-radius:14px;padding:15px;display:grid;grid-template-columns:24px minmax(0,1fr) auto;grid-template-rows:auto auto;column-gap:9px;row-gap:10px;align-items:center;text-align:start;cursor:pointer;color:var(--ink);font:inherit;transition:.2s}.one-page[dir="rtl"] .plan-option{direction:rtl}.one-page[dir="ltr"] .plan-option{direction:ltr}.plan-option:hover{border-color:#e990ae;transform:translateY(-1px)}.plan-option.active{border-color:var(--green);background:#fffbf8;box-shadow:0 4px 0 var(--green)}.radio-dot{width:18px;height:18px;border:2px solid #c9b7c0;border-radius:50%;grid-row:1/3}.active .radio-dot{border:5px solid var(--green)}.plan-data{display:flex;min-width:0;gap:7px;align-items:baseline;white-space:nowrap}.plan-data b{font-size:21px;line-height:1;font-weight:900}.plan-data small{color:#806d78;font-size:13px}.plan-validity{grid-column:2/4;display:flex;min-width:0;align-items:center;gap:6px;color:#796470;font-size:13px;line-height:1.35;white-space:nowrap}.plan-validity .v-icon{flex:0 0 auto}.plan-option>strong{grid-column:3;grid-row:1;font-size:20px;line-height:1;color:var(--green);white-space:nowrap}.plan-option>strong small{font-size:11px}.plan-option em{position:absolute;inset-inline-end:12px;top:-9px;background:var(--coral);color:#fff;border-radius:99px;padding:3px 8px;font-size:10px;font-style:normal;font-weight:800;white-space:nowrap}
...
.checkout-section{padding:34px 0 42px;background:#fff0f5;scroll-margin-top:65px}.checkout-heading{margin-bottom:34px}.embedded-checkout{max-width:1240px;margin:auto}.embedded-checkout :deep(.cart-page){max-width:none!important;min-height:0!important;padding-top:12px!important;padding-bottom:8px!important}.embedded-checkout :deep(.summary-card),.embedded-checkout :deep(.contact-card),.embedded-checkout :deep(.checkout-card),.embedded-checkout :deep(.embedded-payment-card){background:#fff!important;border:0!important;border-block-start:1px solid #e5dcd7!important;border-radius:0!important;box-shadow:none!important}.embedded-checkout :deep(.summary-card),.embedded-checkout :deep(.contact-card){padding:28px 20px 12px!important}.embedded-checkout :deep(.embedded-payment-card){padding-top:28px!important}.embedded-checkout :deep(.contact-column){padding-inline-end:clamp(24px,4vw,56px)!important}.embedded-checkout :deep(.summary-column){padding-inline-start:clamp(24px,4vw,56px)!important;border-inline-start:1px solid #e5dcd7}.embedded-checkout :deep(.v-btn.bg-green-darken-1){background:#d91c58!important;color:#fff!important}.embedded-checkout :deep(.subsection-title){font-family:inherit!important;color:var(--ink)}
...
@media(max-width:600px){.one-page.locale-en .destination-panel>.step-label{display:grid;grid-template-columns:38px minmax(0,1fr);grid-template-rows:auto auto;column-gap:14px;row-gap:5px;align-items:center}.one-page.locale-en .destination-panel>.step-label>i{grid-column:1;grid-row:1 / 3;width:38px;flex:0 0 38px}.one-page.locale-en .destination-panel>.step-label>span{grid-column:2;grid-row:1;font-size:18px;white-space:nowrap}.one-page.locale-en .destination-panel>.step-label>small{grid-column:2;grid-row:2;justify-self:start;margin:0;padding:4px 8px;font-size:11px}}
@media(max-width:600px){.one-page.locale-fr .destination-panel>.step-label>span{font-size:19px;white-space:nowrap}}
@media(max-width:600px){.one-page.locale-fr .package-panel>.step-label{display:grid;grid-template-columns:38px minmax(0,1fr);column-gap:14px;align-items:center}.one-page.locale-fr .package-panel>.step-label>i{width:38px;flex:0 0 38px}.one-page.locale-fr .package-panel>.step-label>span{font-size:18px;white-space:nowrap}}
@media(max-width:360px){.one-page.locale-fr .plans-section>.v-container>.section-heading h2{max-width:285px;font-size:23px}}
@media(max-width:360px){.one-page.locale-en .destination-panel>.step-label>span{font-size:16px}}
@media(max-width:360px){.one-page.locale-fr .destination-panel>.step-label>span{font-size:17px}}
@media(max-width:360px){.one-page.locale-fr .package-panel>.step-label>span{font-size:16px}}
