  ...
  <div class="compatibility-page">
    <v-container class="compatibility-container">
      ...
      <nav class="breadcrumb-row mb-6 mt-2 text-body-2 bidi-text" aria-label="Breadcrumb">
        <router-link to="/ar/" class="text-decoration-none"><span class="text-medium-emphasis">الرئيسية</span></router-link>
        <span class="mx-2"><</span>
        <router-link to="/ar/guides/install-esim-iphone" class="text-decoration-none"><span class="text-medium-emphasis">أدلة eSIM</span></router-link>
        <span class="mx-2"><</span>
      ...
      <div class="mb-8">
        ...
        <v-card rounded="xl" elevation="1" class="pa-5 compatibility-card mb-8">
          <h4 class="subsection-title mb-4 bidi-text">البحث عن هاتف متوافق</h4>
          <v-text-field
            v-model.trim="search"
            label="ابحث حسب العلامة التجارية أو الطراز"
            variant="outlined"
            density="comfortable"
            rounded="lg"
          ...
          <div v-if="!selectedPhone" class="phone-results" role="list">
            ...
            <p v-if="filteredPhoneOptions.length === 0" class="phone-empty">لم يتم العثور على هاتف متوافق</p>
          </div>
          <v-alert v-if="selectedPhone" type="success" variant="tonal" class="mt-4 bidi-text">
            <strong>الهاتف متوافق مع eSIM</strong>
          </v-alert>
          <router-link v-if="selectedPhone" :to="{ path: '/ar/', hash: '#destination-selection' }" class="destination-cta-link">
            <v-btn color="pink" size="large" class="text-none destination-cta" prepend-icon="mdi-earth">عرض الوجهات</v-btn>
          </router-link>
        </v-card>
