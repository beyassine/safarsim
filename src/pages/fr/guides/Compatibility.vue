  ...
  <div class="compatibility-page">
    <v-container class="compatibility-container">
      ...
      <nav class="breadcrumb-row mb-6 mt-2 text-body-2 bidi-text" aria-label="Breadcrumb">
        <router-link to="/fr/" class="text-decoration-none"><span class="text-medium-emphasis">Accueil</span></router-link>
        <span class="mx-2">></span>
        <router-link to="/fr/guides/install-esim-iphone" class="text-decoration-none"><span class="text-medium-emphasis">Guides eSIM</span></router-link>
        <span class="mx-2">></span>
        <strong>Appareils compatibles</strong>
      </nav>

      <div class="mb-8">
        <h1 class="section-title mb-4 bidi-text">Vérifier la compatibilité</h1>
          ...
          <v-col class="" cols="12">
            <h3 class="condition-title bidi-text">
              <v-icon
                ...
                size="large"
              />
              L’appareil prend en charge les eSIM.
            </h3>
          </v-col>
          ...
          <v-col class="" cols="12">
            <h3 class="condition-title bidi-text">
              <v-icon
                ...
                size="large"
              />
              L’appareil n’est pas verrouillé par un opérateur ou par le réseau.
            </h3>
          </v-col>
          ...
          <v-col class="" cols="12">
            <h3 class="condition-title bidi-text">
              <v-icon
                ...
                size="large"
              />
              L’appareil n’est pas jailbreaké (iOS) ou rooté (Android).
            </h3>
          </v-col>
        </v-row>

        <p class="page-text mb-4 bidi-text">
          Vous pouvez consulter notre liste pour vérifier si l’appareil que vous souhaitez utiliser est compatible eSIM. Notez que certains modèles régionaux peuvent ne pas prendre en charge l’eSIM.
        </p>

        <v-card rounded="xl" elevation="1" class="pa-5 compatibility-card mb-8">
          <h4 class="subsection-title mb-4 bidi-text">Rechercher un téléphone compatible</h4>
          <v-text-field
            v-model.trim="search"
            label="Rechercher une marque ou un modèle"
            variant="outlined"
            density="comfortable"
            rounded="lg"
            ...
            hide-details
            clearable
            @update:model-value="selectedPhone = ''"
          />
          <div v-if="!selectedPhone" class="phone-results" role="list">
            <button
            ...
            <p v-if="filteredPhoneOptions.length === 0" class="phone-empty">Aucun téléphone compatible trouvé</p>
          </div>
          <v-alert v-if="selectedPhone" type="success" variant="tonal" class="mt-4 bidi-text">
            <strong>Téléphone compatible avec eSIM</strong>
          </v-alert>
          <router-link v-if="selectedPhone" :to="{ path: '/fr/', hash: '#destination-selection' }" class="destination-cta-link">
            <v-btn color="pink" size="large" class="text-none destination-cta" prepend-icon="mdi-earth">Voir les destinations</v-btn>
          </router-link>
        </v-card>
